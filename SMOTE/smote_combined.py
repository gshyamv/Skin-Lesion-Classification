...
def preprocess_dataset(combined_dataset_path, sample_size=None, resize_dim=256):
    ...
    for class_name in class_folders:
        ...
        for img_file in tqdm(image_files, desc=f"Processing {class_name}"):
            ...
            try:
                ...
                # Convert image to RGB, handling grayscale and images with alpha channel
                ...
                # Apply simple preprocessing (brightness/contrast adjustment)
                ...
                # Store a thumbnail version (to save memory)
    ...
    feature_lengths = [item['features'].shape[0] for item in image_data]
    if len(set(feature_lengths)) > 1:
        raise ValueError(f"Inconsistent feature vector sizes found: {set(feature_lengths)}. "
                         "Ensure all images are processed into the same shape.")
    ...
    # Mapping from index to image_id
    ...
    # Get class-specific image sizes for reconstruction
    class_sizes = {}
...
def apply_tabular_smote(X, y, pca_variance=0.99, min_k_neighbors=3):
    ...
    Uses a two-step PCA:
      1. Preliminary PCA on a random subset to determine the number of components required.
      2. IncrementalPCA (batch processing) on the full dataset.
    """
    ...
    # Step 1: Determine number of components from a random subset
    ...
    # Step 2: Use IncrementalPCA on the full dataset in batches.
    # Adjust batch_size so that it is at least n_components_needed.
    batch_size = max(100, n_components_needed)
    ipca = IncrementalPCA(n_components=n_components_needed, batch_size=batch_size)
    X_reduced = ipca.fit_transform(X)
    print(f"Reduced dimensions from {X.shape[1]} to {X_reduced.shape[1]}")

    # Adjust k_neighbors for SMOTE based on the smallest class count
...
if __name__ == "__main__":
    combined_dataset_path = r'/dist_home/suryansh/dl/Skin-Lesion-Classification/Datasets/combined-dataset '
    output_dir = r'/dist_home/suryansh/dl/Skin-Lesion-Classification/Datasets/combined_balenced-dataset'
