...
class SkinLesionDataset(Dataset):
    ...
    def find_image_file(self, image_id):
        ...
        if os.path.exists(image_path):
            return image_path
        logger.error(f"Image file {image_id}.jpg not found in {self.image_dir}.")
        raise FileNotFoundError(f"Image file {image_id}.jpg not found in {self.image_dir}.")

    def find_mask_file(self, image_id):
        mask_patterns = [
            f"{image_id}_segmentation.png",
            f"{image_id}_mask_mask.png",
        ]
...
class UnifiedJNet(nn.Module):
    def __init__(self, num_classes=7, base_channels=64):
        ...
        # Classification head from segmentation bottleneck
        self.fc = nn.Linear(base_channels*8, num_classes)

        # Precompute "white" for overlay visualization in normalized space
        white_vals = [(1 - m)/s for m, s in zip([0.485, 0.456, 0.406],
                                               [0.229, 0.224, 0.225])]
        self.register_buffer('white', torch.tensor(white_vals).view(1, 3, 1, 1))

    def forward(self, x):
        # Shared segmentation encoder
        ...
        # Classification head: Global Average Pooling of segmentation bottleneck
        gap = seg_features.mean(dim=(2, 3))  # shape: (B, channels)
...
# -------------------------------------------------------------------------
# Helper: Save Overlay Comparison Images for Visualization
# -------------------------------------------------------------------------
...
def train_model(
    ...
    print("=" * 80)
    print("UnifiedJNet: Single model for segmentation & classification using segmentation bottleneck")
    print(f"Optimizer: AdamW (lr={optimizer.param_groups[0]['lr']})")
    print(f"Scheduler: CosineAnnealingLR, Total Epochs: {num_epochs}")
...
# -------------------------------------------------------------------------
# Utility Functions to Save Segmentations for the Dataset
# -------------------------------------------------------------------------
...
def main():
    try:
        ...
        # Set resume flag and path if needed
        resume_training = False
        resume_checkpoint = None  # Set path to checkpoint if resuming
        ...
        image_dir = r"/dist_home/suryansh/dl/Skin-Lesion-Classification/SMOTE/smote_maximum_clarity"
